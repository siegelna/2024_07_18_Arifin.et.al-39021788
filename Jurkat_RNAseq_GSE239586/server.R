...
server <- function(input, output, session) {
  ...
  output$group2 <- renderUI({
    selectInput("group2", "Select Time Point", choices = unique(meta.df$Time_Point)[-1])
  })
...
#   })
  ...
  distribution.plot <- reactive({
    ...
    plot.df <- plot.df %>%
      filter(Time_Point %in% c("0", desired_time_point))

    # Create the distribution plot for the filtered data
    distribution.plot_filtered <- ggplot(plot.df, aes(x = Condition, y = value, fill = Condition)) +
